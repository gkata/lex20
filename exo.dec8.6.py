...
def main():
  ...
  # matrice binaire a partir de M
 
  # 1. iteration sur les lignes et les colonnes, acceder chaque case par ses indices, voir si M[i,j] > 0 et mise a jour -> 1
  # creer autre matrice
  
  lines, columns = M.shape
  Mbin = np.zeros(lines, columns)
  for i in lines:
    for j in columns:
      if M[i,j] > 0:
        Mbin[i,j] = 1

  # 2. sans boucle: selection des elements avec une condition
